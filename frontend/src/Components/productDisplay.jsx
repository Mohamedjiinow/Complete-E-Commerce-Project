...
function Product(){
  ...
  const handleDataRead=()=>{
    axios.get(`http://localhost:9000/read/Products?page=${page}`).then((res)=>{
      setData(res.data)
    })
  }
  useEffect(()=>{
  ...
  const handledelete=(id)=>{
    ...
    axios.delete(`http://localhost:9000/delete/product/${id}`).then(()=>{
      ...
      toast("this product has been deleted!", {
          ...
          handleDataRead()
            ...
            const handlesearchData= (id)=>{
              ...
              if (key){ 
              axios.get(`http://localhost:9000/search/Products/${key}`).then((res)=>{
                setData(res.data)
              }).catch((Error)=>{console.log(Error) })
            }
            else{
